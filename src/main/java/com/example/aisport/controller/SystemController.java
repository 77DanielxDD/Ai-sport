...
import com.example.aisport.service.ObjectStorageService;
...
public class SystemController {
    ...
    public ResponseEntity<?> health() {
        ...
        Map<String, Object> storage = checkObjectStorage();

        Map<String, Object> checks = new LinkedHashMap<>();
        checks.put("database", db);
        checks.put("rabbitmq", mq);
        checks.put("python_ai", py);
        checks.put("object_storage", storage);

        boolean dbUp = "UP".equals(db.get("status"));
        boolean pyUp = "UP".equals(py.get("status"));
        boolean mqUp = "UP".equals(mq.get("status"));
        boolean storageUp = "UP".equals(storage.get("status")) || "DISABLED".equals(storage.get("status"));

        boolean coreUp = dbUp && pyUp;
        String status = coreUp ? ((mqUp && storageUp) ? "UP" : "DEGRADED") : "DOWN";

        Map<String, Object> body = new HashMap<>();
        body.put("status", status);
        body.put("checks", checks);
        body.put("degradedReason", buildDegradedReason(mqUp, storageUp));
    ...
    private void enrichDbReadiness(Map<String, Object> out) {
        try {
            String dbName = jdbcTemplate.queryForObject("SELECT DATABASE()", String.class);
            out.put("databaseName", dbName);
            ...
            int tableCount = found == null ? 0 : found;
            out.put("requiredTablesFound", tableCount);
            out.put("requiredTablesReady", tableCount == 4);
        } catch (Exception ignored) {
            out.put("requiredTablesReady", null);
            out.put("requiredTablesFound", null);
        }
    }
    ...
    private Map<String, Object> checkObjectStorage() {
        Map<String, Object> result = new HashMap<>();
        if (!objectStorageService.isEnabled()) {
            result.put("status", "DISABLED");
            result.put("detail", "Object storage disabled by config");
            return result;
        }
        boolean reachable = objectStorageService.isBucketReachable();
        result.put("status", reachable ? "UP" : "DOWN");
        result.put("detail", reachable ? "Bucket reachable" : "Bucket unreachable");
        result.put("bucket", objectStorageService.getBucket());
        return result;
    }

    private String buildDegradedReason(boolean mqUp, boolean storageUp) {
        if (mqUp && storageUp) {
            return null;
        }
        if (!mqUp && !storageUp) {
            return "RabbitMQ unavailable and object storage unavailable";
        }
        if (!mqUp) {
            return "RabbitMQ unavailable, local async fallback enabled";
        }
