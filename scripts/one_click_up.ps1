...
if ([string]::IsNullOrWhiteSpace($CosBucket)) {
    $CosBucket = $env:DEV_OBJECT_STORAGE_BUCKET
}
...
if ([string]::IsNullOrWhiteSpace($CosPublicBaseUrl)) {
    if (-not [string]::IsNullOrWhiteSpace($env:DEV_OBJECT_STORAGE_PUBLIC_BASE_URL)) {
        $CosPublicBaseUrl = $env:DEV_OBJECT_STORAGE_PUBLIC_BASE_URL
    } elseif (-not [string]::IsNullOrWhiteSpace($CosBucket)) {
...
if (-not (Test-Path $aiDir)) {
    throw "AI service directory not found: $aiDir"
}
New-Item -ItemType Directory -Force -Path (Join-Path $aiDir ".mplconfig") | Out-Null
Start-ManagedProcess -Name "ai" -FilePath "cmd.exe" -ArgumentList @(
    "/c",
    "set `"MPLCONFIGDIR=$aiDir\.mplconfig`" && `"$PythonExe`" -m uvicorn app.main:app --host 127.0.0.1 --port $AiPort"
